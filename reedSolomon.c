 ...
 * Exp table doubled for easy modular arithmetic
 */
static const unsigned char GfExp[512] = {
...
rsEncode(
 ...
 ,unsigned int m
  ...
  for (i = 0; i < m; ++i) {
    ...
    for (b = 0; b < l; ++b) {
      c = 0;
      /* Linear combination of data shards using Cauchy matrix */
      for (j = 0; j < k; ++j)
        c ^= gfMul(d[j][b], coef[j]);
      p[i][b] = c;
    }
  }
  return (0);
